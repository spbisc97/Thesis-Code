...
class Satellite_base(gym.Env):
    ...
    def __init__(self, render_mode=None, observation_space="MlpPolicy",control="Ml"):
            ...
            self.observation_space = spaces.Dict(
                {
                    ...
                    "fuel": spaces.Box(low=0, high=Chaser.initial_fuel_mass, shape=(1,), dtype=np.float32),
                }
            )
    ...
    def _get_observation(self):
        if isinstance(self.observation_space, spaces.Box):
            return np.concatenate((self.chaser.state, np.asarray((self.chaser.fuel_mass,), dtype=np.float32)))
        else:
            return {
    ...
    def _reward_function(self):
        ...
        reward += 1/(self.chaser.distance_to_target()+1)

        return float(reward)

    def _shape_reward(self):
        shaping = (-self.chaser.distance_to_target()
                   + 1/(self.chaser.velocity_norm()+1)
                   + 10/(self.chaser.distance_to_target()+1))
        return shaping
...
class Chaser():
    ...
    def _Sat_Translational_Dyn(self, y, t, trust):
        ...
        dy[3] = self.w**2*y[0]+2*self.w*y[4]+self.mu / \
            (self.rt**2)-(eta)*(self.rt+y[0])+torques[0]/total_mass
        dy[4] = -2*self.w*y[3]+self.w**2*y[1]-(eta)*y[1]+torques[1]/total_mass
    ...
    def move(self, trust):
        ...
        self.state = self._rk4(self._Sat_Translational_Dyn,
                               self.state, self.dt, args=(trust,))
        self.fuel_mass = self.fuel_mass - \
